    ...
    <x-top-bar>
        <div class="flex items-center">
            ...
            <div class="ml-2 bg-white block mb-2">
                <div class="font-bold text-xl">Thread</div>
            </div>
        </div>
    </x-top-bar>
    <div class="px-4 pt-4 border-l border-r border-b border-gray-300">
        <div class="flex items-center mb-3">
            <a href="{{route('profile', ['user' => $tweet->user->username])}}" class="flex-shrink-0 w-9 mr-2">
        ...
        <div class="text-xl mb-3" style="max-width:620px">{{$tweet->body}}</div>
    ...
    <section class="mb-6 load-tweets">
        <div class="load-tweets-ajax dynamic-load">
            ...
            <div class="flex py-3 border-l border-r border-b border-gray-300">
                <a href="{{route('profile', ['user' => $replyTweet->user->username])}}"
                    class="flex-shrink-0 w-9 ml-4 mr-2">
                    <x-avatar-icon>{{$replyTweet->user->avatar}}</x-avatar-icon>
                </a>
                <div class="ml-1 mr-2 w-full tweet-h">
                    <span class="font-bold"><a href="{{route('profile', ['user' => $replyTweet->user->username])}}">{{$replyTweet->user->name}}</a></span>
                    ...
                    <div class="word-break">
                        @if(isset($replyTweet->reply))
                            <div class="text-gray-600"> Replying to <span class="text-blue-900">
                                <a href="{{route('profile', ['user' => $tweet->user->username])}}">{{ '@' . $tweet->user->username}}</a></span>
                            </div>
        ...
        @empty
        <div class="text-lg text-gray-600 text-center py-3 border-l border-r border-b border-gray-300">
            Nothing here yet
        </div>
        @endforelse
